...
*
* Batching added by Spencer Easton 2015
*/
var fs = require('fs'),
    https = require('https'),
...
app.post('/', function (req, res) {
  var q = req.body;
  var payloads = q.payloads;
  var key = q.key;
  var algorithm = q.algorithm;
  var JWTs = {};
  for(var i = 0; i < payloads.length; i++){
    JWTs[payloads[i].user]={"claim":jwt.encode(payloads[i].claim, key, algorithm),"expire":payloads[i].expire};
  }
  res.send(JWTs);
});
...
app.post('/checkjwt', function (req, res) {
  var q = req.body;
  var enc = jwt.decode(q.token, q.key, q.noVerify, q.algorithm);
  res.send(enc);
});
